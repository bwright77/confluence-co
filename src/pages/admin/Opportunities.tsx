...
import type { Opportunity, ScoreDetail } from '../../lib/adminTypes'

type TabFilter = 'grants' | 'discovered'
...
// Full status list (including terminal) for the filter dropdown
...
const STATUS_LABELS: Record<string, string> = Object.fromEntries(
  [...GRANT_STATUSES, { id: 'grant_discovered', label: 'Discovered' }].map(s => [s.id, s.label])
)
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="text-xs font-medium text-white bg-cc-sage-ink hover:bg-cc-sage-ink/90 px-3 py-1.5 rounded-lg transition-colors"
                      >
                        Add to Pipeline
                      </button>
                    </div>
...
export default function Opportunities() {
  useEffect(() => {
    document.title = 'Opportunities · Confluence Colorado admin'
  }, [])

  const queryClient  = useQueryClient()
  const { session }  = useAuth()
  const [searchParams, setSearchParams] = useSearchParams()
  const tab          = (searchParams.get('tab') === 'discovered' ? 'discovered' : 'grants') as TabFilter
  const statusFilter =  searchParams.get('status') ?? ''

  function setTab(t: TabFilter) {
    // Switching tabs clears status filter
    setSearchParams(t === 'grants' ? {} : { tab: 'discovered' }, { replace: true })
  }
  function setStatus(s: string) {
    const params: Record<string, string> = {}
    if (tab !== 'grants') params.tab = tab
  ...
  // Pipeline opps: exclude the discovered review queue
  ...
  const filtered = pipelineOpps.filter(o => {
    if (statusFilter === 'active') {
      if (INACTIVE_GRANT_STATUSES.includes(o.status)) return false
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-cc-sage-ink">Opportunities</h1>
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
          <button
            ...
            className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${
              tab === 'grants' ? 'bg-white text-navy shadow-sm' : 'text-gray-500 hover:text-navy'
            }`}
          >
            Grants
          </button>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-2.5">
                      <div>
                        ...
                        {o.funder && (
                          <p className="text-xs text-gray-400 mt-0.5">{o.funder}</p>
                        )}
                      </div>
                    </div>
                  </td>
