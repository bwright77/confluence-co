...
import type { BoardMeeting, BoardMeetingExtractedData } from '../../lib/adminTypes'
...
function FieldRow({ label, htmlFor, children }: { label: string; htmlFor?: string; children: React.ReactNode }) {
  return (
    <div className="grid grid-cols-[140px_1fr] gap-3 items-start">
      <label htmlFor={htmlFor} className="text-xs text-gray-500 pt-2.5 leading-tight">{label}</label>
      <div>{children}</div>
    </div>
  )
}
...
function TextArea({
  value, onChange, rows = 3, placeholder, id,
...
export default function BoardMeetingDetail() {
  ...
  // ── Document title ────────────────────────────────────────────
  useEffect(() => {
    document.title = meeting?.meeting_date
      ? `${format(new Date(meeting.meeting_date + 'T12:00:00'), 'MMMM d, yyyy')} · Board meeting · Confluence Colorado admin`
      : 'Board meeting · Confluence Colorado admin'
      ...
      {activeData && !isProcessing && (
        <div className="space-y-3">
          ...
          <Section title="Meeting Info">
            <FieldRow label="Date" htmlFor="mi-date">
              <TextInput
                id="mi-date"
                type="date"
            ...
            <FieldRow label="Start Time" htmlFor="mi-start">
              <TextInput
                id="mi-start"
                type="time"
            ...
            <FieldRow label="End Time" htmlFor="mi-end">
              <TextInput
                id="mi-end"
                type="time"
            ...
            <FieldRow label="Location" htmlFor="mi-location">
              <TextInput
                id="mi-location"
            ...
            <FieldRow label="Called to Order By" htmlFor="mi-order">
              <TextInput
                id="mi-order"
          ...
          <Section title="Attendance">
            <FieldRow label="Directors Present" htmlFor="att-present">
              <ArrayTextArea
                id="att-present"
            ...
            <FieldRow label="Directors Absent" htmlFor="att-absent">
              <ArrayTextArea
                id="att-absent"
            ...
            <FieldRow label="Guests" htmlFor="att-guests">
              <ArrayTextArea
                id="att-guests"
            ...
            <FieldRow label="Quorum" htmlFor="att-quorum">
              <div className="flex items-center gap-4">
                {(['true', 'false', 'unknown'] as const).map(opt => (
                  <label key={opt} className="flex items-center gap-1.5 text-sm cursor-pointer">
                    <input
                      id={opt === 'true' ? 'att-quorum' : undefined}
                      type="radio"
                      name="quorum"
                      value={opt}
                      checked={
          ...
          <Section title="Prior Minutes">
            <FieldRow label="Reviewed" htmlFor="pm-reviewed">
            ...
            <FieldRow label="Approved" htmlFor="pm-approved">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  id="pm-approved"
                  type="checkbox"
            ...
            <FieldRow label="Corrections" htmlFor="pm-corrections">
              <TextArea
                id="pm-corrections"
              ...
              {activeData.reports.map((report, idx) => (
                <div key={idx} className="border border-gray-100 rounded-lg p-3 space-y-2">
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label htmlFor={`report-${idx}-title`} className="block text-xs text-gray-400 mb-1">Title</label>
                      <TextInput
                        id={`report-${idx}-title`}
                        value={report.title}
                        onChange={v => {
                    ...
                    <div>
                      <label htmlFor={`report-${idx}-presenter`} className="block text-xs text-gray-400 mb-1">Presenter</label>
                      <TextInput
                        id={`report-${idx}-presenter`}
                  ...
                  <div>
                    <label htmlFor={`report-${idx}-summary`} className="block text-xs text-gray-400 mb-1">Summary</label>
                    <BufferedTextArea
                      id={`report-${idx}-summary`}
            ...
            <Section title="Motions">
              {activeData.motions.map((motion, idx) => (
                <div key={idx} className="border border-gray-100 rounded-lg p-3 space-y-2">
                  ...
                  <div>
                    <label htmlFor={`motion-${idx}-desc`} className="block text-xs text-gray-400 mb-1">Motion Text</label>
                    <TextArea
                      id={`motion-${idx}-desc`}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label htmlFor={`motion-${idx}-moved`} className="block text-xs text-gray-400 mb-1">Moved By</label>
                      <TextInput
                        id={`motion-${idx}-moved`}
                    ...
                    <div>
                      <label htmlFor={`motion-${idx}-seconded`} className="block text-xs text-gray-400 mb-1">Seconded By</label>
                      <TextInput
                        id={`motion-${idx}-seconded`}
                  ...
                  <div>
                    <label htmlFor={`motion-${idx}-vote-yes`} className="block text-xs text-gray-400 mb-1">Vote</label>
                    <div className="grid grid-cols-4 gap-2">
                      {(['yes', 'no', 'abstain'] as const).map(k => (
                        <div key={k}>
                          <label htmlFor={`motion-${idx}-vote-${k}`} className="block text-xs text-gray-400 mb-1 capitalize">{k}</label>
                          <input
                            id={`motion-${idx}-vote-${k}`}
                            type="number"
                            min={0}
                            value={motion.vote[k] ?? ''}
                            onChange={e => {
                      ...
                      <div>
                        <label htmlFor={`motion-${idx}-vote-result`} className="block text-xs text-gray-400 mb-1">Result</label>
                        <TextInput
                          id={`motion-${idx}-vote-result`}
          ...
          <Section title="Next Meeting" defaultOpen={false}>
            <FieldRow label="Date" htmlFor="nm-date">
              <TextInput
                id="nm-date"
                type="date"
            ...
            <FieldRow label="Time" htmlFor="nm-time">
              <TextInput
                id="nm-time"
                type="time"
            ...
            <FieldRow label="Location" htmlFor="nm-location">
              <TextInput
                id="nm-location"
          ...
          <Section title="Adjournment" defaultOpen={false}>
            <FieldRow label="Adjourned At" htmlFor="adj-time">
              <TextInput
                id="adj-time"
                type="time"
